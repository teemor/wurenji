  ...
  if (window.layui) {
    layui.use(['laytpl', 'element'], function () {
      ...
      var zoneTpl = [
        ...
        '{{# } }}',
        '</div><div class="bar-text">{{ zone.popup.bar.text }}<span class="bar-num">{{ zone.popup.bar.used }}</span>{{ zone.popup.bar.unit1 }} {{ zone.popup.bar.text2 }}<span class="bar-num">{{ zone.popup.bar.available }}</span>{{ zone.popup.bar.unit2 }}</div></div>',
        '{{# } }}',
        '</div></div>',
        '</div>',
        '{{# }); }}',
